 ...
 *   分析：一般常见的Item类型主要有：文字、图片、图文、多个Item。其中文字和图片可以使用系统提供的【initWithImage】和【initWithTitle】来快速构建，但是通过这两种方式创建的Item的缺点比较多：
        ...
        --图片，文字无法保持本身的颜色，都会显示tintColor。虽然可以通过    self.navigationController.navigationBar.tintColor = [UIColor whiteColor];
          来修改文字、图片颜色，但是图片仅限于那种线框图如"<"类型的(补充：图片的话可以设置UIImageRenderingModeAlwaysOriginal模式来显示原图，可参考个人github项目)
